...
function createLifeScene() {
    ...
    // Create shader-based life terrain using the life.glsl shader
    const terrainGeometry = new THREE.PlaneGeometry(80, 80, 128, 128);
    
    const lifeShaderMaterial = new THREE.ShaderMaterial({
        uniforms: {
            u_time: { value: 0 },
    ...
    const terrain = new THREE.Mesh(terrainGeometry, lifeShaderMaterial);
    ...
    group.userData.animate = () => {
        ...
        lifeShaderMaterial.uniforms.u_time.value = time;
        lifeShaderMaterial.uniforms.u_audio.value = audioAnalyzer ? audioAnalyzer.getAverageFrequency() / 255 : 0;
